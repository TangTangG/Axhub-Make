...
function normalizeTaskRef(value: unknown): Partial<ExternalEditingTaskRef> | null {
  ...
  return {
    ...
    error: hasText(record.error) ? record.error.trim() : null,
    code: hasText(record.code) ? record.code.trim() : null,
    output: hasText(record.output) ? record.output.trim() : null,
    ...(record.chunk !== undefined ? { chunk: record.chunk } : {}),
    ...(record.details !== undefined ? { details: record.details } : {}),
  };
}

function normalizeTargetRef(value: unknown): CommentaryExternalEditingTargetRef | null {
  if (!value || typeof value !== 'object') return null;
  const record = value as Record<string, unknown>;
  return {
    locator: record.locator && typeof record.locator === 'object'
      ? (record.locator as CommentaryExternalEditingTargetRef['locator'])
      : null,
    label: hasText(record.label) ? record.label.trim() : null,
  };
}
  ...
  async function handleEditorEditingSetRequest(message: IntegrationWsMessage): Promise<void> {
    ...
    sendMessage({
      ...
      payload: await options.setNodeEditingState(
        ...
        normalizeTargetRef(payload.targetRef),
      ),
    });
  }
