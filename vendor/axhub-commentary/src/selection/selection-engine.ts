...
/** Text-bearing tags that should be selectable only when they own direct text. */
const DIRECT_TEXT_TARGET_TAGS = new Set([
  'H1',
  'H2',
  'H3',
  'H4',
...
function isDirectTextTargetTag(tag: string): boolean {
  return DIRECT_TEXT_TARGET_TAGS.has(tag.toUpperCase());
}

function isDirectTextTarget(element: Element): boolean {
  return isDirectTextTargetTag(element.tagName) && hasDirectNonWhitespaceText(element);
}
...
export function createSelectionEngine(options: SelectionEngineOptions): SelectionEngine {
  ...
  function scoreElement(
    ...
    // Common text elements with their own text are stable, user-intentful targets.
    if (isDirectTextTarget(element)) {
      score += 5;
      reasons.push('text:direct:+5');
    }
  ...
  function findBestTargetFromEvent(event: Event, modifiers: Modifiers): Element | null {
    ...
    if (
      directCandidate
      && !directCandidate.wrapperOnly
      && (!isDirectTextTargetTag(directHit.tagName) || isDirectTextTarget(directHit))
    ) {
      base = directHit;
    } else {
