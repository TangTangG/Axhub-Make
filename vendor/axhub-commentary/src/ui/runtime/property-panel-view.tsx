...
import {
  ShortcutCaptureCard,
  shortcutCaptureHintStyle,
} from './shortcut-capture-card';
...
const AGENT_WAKE_FAILURE_MESSAGE =
  'AI 唤醒失败，请在终端执行 npx @axhub/acp@latest，再重试';
const AGENT_WAKE_TIMEOUT_MS = 12000;
const AGENT_INTERRUPT_TIMEOUT_MS = 12000;
const COMMENTARY_CLIENT_SKILL_URL =
  'https://github.com/lintendo/Axhub-Skills/blob/main/skills/axhub-commentary-client/SKILL.md';
...
function normalizeCommentarySkillIds(
  value: unknown,
...
export function buildSkillInstallPrompt(
  ...
  return [
    ...
    ...(resolvedSkillInstallSource
      ? ['', '宿主补充技能来源：', resolvedSkillInstallSource]
      : []),
    '',
  ...
  const providerOptions = Array.isArray(record.providerOptions)
    ? record.providerOptions
        .map((item) => {
          ...
          const label =
            normalizeAiExecutionWorkspacePath(itemRecord.label) || optionValue;
          return {
            value: optionValue,
            label,
        ...
        .filter(
          (item): item is { value: string; label: string; disabled: boolean } =>
            Boolean(item),
        )
    : undefined;
  return {
...
const readLocalDirectoryBrowserResult = (
  value: unknown,
): LocalDirectoryBrowserState | null => {
  ...
  const roots = Array.isArray(record.roots)
    ? record.roots
        .map((item) => normalizeAiExecutionWorkspacePath(item))
        .filter(Boolean)
    : [];
...
export const PropertyPanelView = React.forwardRef<
  PropertyPanelHandle,
...
>(function PropertyPanelView(props, ref) {
  ...
  const hostToolbarListenersRef = React.useRef<
    Set<(state: CommentaryHostToolbarState) => void>
  >(new Set());
  ...
  const [agentPromptInterrupting, setAgentPromptInterrupting] =
    React.useState(false);
  const [agentWakeChecking, setAgentWakeChecking] = React.useState(false);
  const [sessionActivityCardOpen, setSessionActivityCardOpen] =
    React.useState(false);
  ...
  const [pagePanelPosition, setPagePanelPosition] =
    React.useState<FloatingPosition | null>(options.initialPosition ?? null);
  const [toolbarDragging, setToolbarDragging] = React.useState(false);
  const [viewportSize, setViewportSize] = React.useState(() => ({
    width: window.innerWidth,
    height: window.innerHeight,
  }));
  const [compactAnchorRect, setCompactAnchorRect] =
    React.useState<RectLike | null>(null);
  const [shortcutDialogOpen, setShortcutDialogOpen] = React.useState(false);
  const [shortcutDraft, setShortcutDraft] =
    React.useState<CommentShortcutSettings>(
      options.getCommentShortcutSettings?.() ?? {
        ...DEFAULT_COMMENT_SHORTCUT_SETTINGS,
      },
    );
  const [capturingShortcutIndex, setCapturingShortcutIndex] = React.useState<
    number | null
  >(null);
  const [panelRefreshKey, setPanelRefreshKey] = React.useState(0);
  const [tweakRevision, setTweakRevision] = React.useState(0);
  const [agentPromptSendingElementKey, setAgentPromptSendingElementKey] =
    React.useState<string | null>(null);
  const [settingsPopoverOpen, setSettingsPopoverOpen] = React.useState(false);
  const [commentarySkillDialogOpen, setCommentarySkillDialogOpen] =
    React.useState(false);
  const [commentarySkillDraftIds, setCommentarySkillDraftIds] = React.useState<
    string[]
  >(() =>
  ...
  const [commentarySkillSaving, setCommentarySkillSaving] =
    React.useState(false);
  ...
  const [agentProviderRefreshPending, setAgentProviderRefreshPending] =
    React.useState(false);
  ...
  const showCommentarySkillSettings =
    ...
    (Boolean(options.onHostToolbarAction) ||
      (options.commentarySkillOptions?.length ?? 0) > 0);
  const [aiExecutionProvider, setAiExecutionProvider] = React.useState(() =>
    normalizeAiExecutionProvider(options.aiExecutionProvider),
  );
  const [aiExecutionWorkspacePath, setAiExecutionWorkspacePath] =
    React.useState(() =>
      normalizeAiExecutionWorkspacePath(options.aiExecutionWorkspacePath),
    );
  const [aiExecutionRunConcurrency, setAiExecutionRunConcurrency] =
    React.useState(() =>
      normalizeAiExecutionRunConcurrency(options.aiExecutionRunConcurrency),
    );
  const [aiExecutionProviderOptionsState, setAiExecutionProviderOptionsState] =
    React.useState<Array<{ value: string; label: string; disabled?: boolean }>>(
      () => [...AI_EXECUTION_PROVIDER_OPTIONS],
    );
  const [aiExecutionConfigBusy, setAiExecutionConfigBusy] =
    React.useState(false);
  ...
  const showPropertyPanelToolbarButton =
    propertyPanelVisible && hasPageTweakEntries;
  ...
  const visibleTerminalTaskCount = hideExecutionControls
    ? 0
    : visibleExecutionTerminalTaskCount;
  const currentAgentConversation =
    options.getCurrentAgentConversationState?.() ?? null;
  ...
  React.useEffect(() => {
    ...
    return options.subscribeSessionActivity(sessionActivityTarget, (item) => {
      setSessionActivities((previous) =>
        appendRecentSessionActivities(previous, item),
      );
    });
  ...
  const clampToViewport = React.useCallback(
    (
      ...
      const height =
        sizeOverride?.height ??
        rect?.height ??
        (toolMinimized ? COMPACT_TOOL_SIZE : 72);
  ...
  const applyPanelPosition = React.useCallback(
    (nextPosition: FloatingPosition | null) => {
      pagePanelPositionRef.current = nextPosition
        ? clampPagePanelToViewport(nextPosition)
        : null;
  ...
  const handleShortcutDraftChange = React.useCallback(
    (updater: (prev: CommentShortcutSettings) => CommentShortcutSettings) => {
      setShortcutDraft((prev) =>
        sanitizeCommentShortcutSettings(updater(prev)),
      );
    },
    [],
  );
  ...
  const currentTaskSessionHref =
    ...
    (currentAgentTask?.sessionId
      ? `/session/${currentAgentTask.sessionId}`
      : '');
  ...
  const inlineSendVisible =
    !hideExecutionControls && agentPromptToolbarAction.sendVisible;
  ...
  const clearAllEditsToolbarButton = clearAllEditsDisabled ? (
    ...
    <Popconfirm
      ...
      onConfirm={() =>
        runAction(() => options.onClearEdits?.({ skipConfirm: true }))
      }
    >
  ...
  const agentProviderSettingsMenuItems: NonNullable<MenuProps['items']> =
  ...
  const agentProviderDropdownItems: NonNullable<MenuProps['items']> =
  ...
  const handleAgentProviderMenuClick = React.useCallback<
  ...
  const selectedAgentMenuLabel = uiSettings.agentProvider
    ? (AGENT_MENU_OPTIONS.find(
        (item) => item.value === uiSettings.agentProvider,
      )?.label ?? uiSettings.agentProvider)
    : '默认';
  ...
  const applyAiExecutionConfigResult = React.useCallback((result: unknown) => {
    ...
    if (
      resultRecord &&
      Object.prototype.hasOwnProperty.call(resultRecord, 'provider')
    ) {
      setAiExecutionProvider(
    ...
    if (
      resultRecord &&
      Object.prototype.hasOwnProperty.call(resultRecord, 'workspacePath')
    ) {
      setAiExecutionWorkspacePath(
    ...
    if (
      resultRecord &&
      Object.prototype.hasOwnProperty.call(resultRecord, 'runConcurrency')
    ) {
      setAiExecutionRunConcurrency(
  ...
  const aiExecutionProviderLabel =
    aiExecutionProviderOptions.find(
  ...
  const aiExecutionConfigConfigured =
    Boolean(aiExecutionWorkspacePath) ||
    options.aiExecutionConfigConfigured === true;
  ...
  const aiExecutionConfigSettingsItem: SettingsItem | null =
    hideExecutionControls || !options.onHostToolbarAction
      ? null
      : {
	                    ...
	                    suffix={
	                      ...
	                      >
	                        <Button
	                          ...
	                          disabled={
	                            aiExecutionConfigBusy || directoryPickerBusy
	                          }
	                          onMouseDown={(event) => {
	                            event.preventDefault();
	                            event.stopPropagation();
  ...
  const propertyPanelSettingsItem: SettingsItem | null =
              ...
              onChange={(value) => {
                onUiSettingsChange({
                  ...
                  agentRunConcurrency:
                    typeof value === 'number'
                      ? value
                      : uiSettings.agentRunConcurrency,
                });
              }}
              style={{ width: 64 }}
            />
  ...
  const commentarySkillSettingsItem: SettingsItem | null =
    showCommentarySkillSettings
      ? {
  ...
  const settingsItems: SettingsItem[] = [
    ...
    ...(agentRunConcurrencySettingsItem
      ? [agentRunConcurrencySettingsItem]
      : []),
    ...(!options.onHostToolbarAction && commentarySkillSettingsItem
      ? [commentarySkillSettingsItem]
      : []),
    {
    ...
    >
        ...
        >
          <Button
            ...
            style={{
              ...
              width: 32,
              padding: 0,
              color: uiSettings.darkMode
                ? EDITOR_CHROME.accent
      ...
      <div style={{ display: 'flex', flexDirection: 'column' }}>
        {settingsItems.map((item) => (
          ...
          >
            {item.fullWidth ? (
              <div style={{ width: '100%' }}>{item.control}</div>
            ) : (
              <>
                <span
                  style={{
  ...
  const hostToolbarState = React.useMemo<CommentaryHostToolbarState>(() => {
    const agentOptions = [
      ...
      ...AGENT_MENU_OPTIONS.map((item) => {
        const availability =
          agentProviderAvailabilityMap.get(item.value) ?? null;
        return {
          value: item.value,
          label: item.label,
    ...
    const annotationEnableAvailable =
      options.getAnnotationEnableAvailable?.() ?? false;
    const annotationEnableLoading =
      options.getAnnotationEnableLoading?.() ?? false;
    ...
    return {
      ...
      annotationEnableTitle: annotationEnabled
        ? '需求标注已开启'
        : '开启需求标注',
    };
  }, [
      ...
      switch (action.type) {
        ...
        case 'toggle-dark-mode':
          onUiSettingsChange({
            ...uiSettings,
            darkMode:
              typeof action.darkMode === 'boolean'
                ? action.darkMode
                : !uiSettings.darkMode,
          });
          return true;
  ...
  React.useImperativeHandle(
    ...
    () => ({
      ...
      getPosition() {
        return showExpandedPanel
          ? pagePanelPositionRef.current
      ...
      >
        ...
        >
          <div style={{ display: 'flex', flexDirection: 'column', gap: 10 }}>
            {commentarySkillOptions.map((skill) => {
        ...
        >
          <div style={{ display: 'flex', flexDirection: 'column', gap: 16 }}>
            ...
            <div style={shortcutCaptureHintStyle}>
              ...
              {COMMENT_SHORTCUT_LONG_PRESS_MS}ms 触发。
            </div>
        ...
        >
          ...
          >
            ...
            {directoryPickerState?.roots.length ? (
              <div style={{ display: 'flex', gap: 8, flexWrap: 'wrap' }}>
                {directoryPickerState.roots.map((rootPath) => (
                  <Button
                    key={rootPath}
                    size="small"
                    disabled={directoryPickerBusy}
