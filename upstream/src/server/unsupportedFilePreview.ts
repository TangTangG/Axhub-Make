...
export type UnsupportedPreviewResourceType = 'docs' | 'templates';
...
function isInlinePreviewableExtension(ext: string): boolean {
  return TEXT_EXTENSIONS.has(ext) || IMAGE_EXTENSIONS.has(ext);
}

function isBrowserFilePreviewRequest(req: IncomingMessage): boolean {
...
function shouldRenderUnsupportedFilePreview(req: IncomingMessage, filePath: string): boolean {
...
function renderUnsupportedFilePreviewHtml(params: {
  ...
  filePath: string;
  openEndpoint: string;
  resourceType: UnsupportedPreviewResourceType;
}): string {
