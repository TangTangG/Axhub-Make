...
import { getProjectExportsDir, type StoredProjectRecord } from './projectCore/index.ts';

export interface AxhubPublishBinding {
  pid: number;
  path: string;
  url: string;
  projectId: string;
...
export function readLatestAxhubPublishBinding(
  projectRoot: string,
  context: { projectId: string; prototypeId: string },
): AxhubPublishBinding | null {
  let latest: AxhubPublishBinding | null = null;
  ...
  for (const record of readExportRecords(projectRoot)) {
    ...
    if (
      record.operationType !== 'cloud.publish.axhub'
