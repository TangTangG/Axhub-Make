...
- `@axhub/annotation`：只由 `client` 使用，并且已经发布为 npm 包，client 直接依赖 `@axhub/annotation@1.x`，跟随 1 大版本内的最新兼容版本。
...
`@axhub/annotation` 不走 vendor 流程。升级这类 client npm 依赖时，应先发布对应 npm 版本，再在本仓库根目录运行 `pnpm update @axhub/annotation --lockfile-only --filter @axhub/make-client` 同步 `client` 依赖锁定。
