...
- generated user-project Git ignore rules and template-seed metadata documentation;
...
The source checkout keeps `.axhub/make/comments/` and `.axhub/make/comment-assets/` ignored so local review data cannot be staged accidentally. During ZIP assembly, the release helper adds the matching negation rules to the packaged `.gitignore`, allowing generated user projects to version their shared comments without exposing the publishing repository's local data.
...
- `scripts/release-make.mjs` so the packaged `.gitignore` enables shared comment tracking while the source checkout remains protected.
...
4. source-checkout verification proving live comment data remains ignored;
5. ZIP inspection proving `handle-comments` is present, `prototype-comments` is absent, and the packaged `.gitignore` enables shared comment tracking;
6. ZIP inspection proving real comment data, comment assets, project PRDs, local prototypes, and Trae are absent;
7. generated sidebar inspection against the manifest-selected prototypes and themes;
8. manifest version, release notes, URLs, and SHA-256 consistency.
