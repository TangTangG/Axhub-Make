...
1. 找到目标原型：`apps/make-server/client/src/prototypes/<prototype-id>/`。
...
8. 按改动范围运行验证，通常是：
   ```bash
   pnpm --dir apps/make-server/client typecheck
   node apps/make-server/client/scripts/check-app-ready.mjs /prototypes/<prototype-name>
   ```
