...
export function generateSvgContent(screenshotUrl: string, config: ImageConfig, title: string, label: string) {
  ...
  <g id="textGroup" transform="translate(${cx},${cy})">
    <text id="mainTitle" x="0" y="-12" text-anchor="middle" font-family="Arial, Helvetica, sans-serif" font-size="28" font-weight="700" fill="#333">${title}</text>
    <text id="description" x="0" y="18" text-anchor="middle" font-family="Arial, Helvetica, sans-serif" font-size="14" font-weight="400" fill="#999">Axhub Runtime ${label}</text>
  </g>
</svg>`;
    }
