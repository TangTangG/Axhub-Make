...
describe('ACP host frontend regression script source', () => {
  ...
  it('covers AI active canvas writes instead of artifact postMessage queries', () => {
    ...
    expect(source).toContain('insert_canvas_ai_node_for_active_canvas_write');
    expect(source).toContain('submit_canvas_ai_prompt_for_active_canvas_write');
    expect(source).toContain('write_active_canvas_artifacts_for_realtime_flow');
    expect(source).toContain("document.dispatchEvent(new CustomEvent('axhub:insertAiGeneration'");
    ...
    expect(source).toContain('textarea[aria-label="AI 生成提示词"]');
    expect(source).toContain('button[aria-label="发送"]');
    expect(source).toContain("element.customData?.type === 'axhub-ai-generation'");
    expect(source).toContain("element.customData?.scene === 'document'");
  ...
  it('has a real ACP sidebar canvas artifact regression without a mock sidebar', () => {
    ...
    expect(runner).toContain('不要寻找顶部 AI 添加节点入口；这个入口已经被移除。');
    expect(runner).toContain('visual_open_canvas_ai_composer_from_inserted_node');
    ...
    expect(runner).toContain('openCanvasAiComposerFromUnifiedInsertEvent');
    expect(runner).toContain('CANVAS_AI_COMPOSER_ROOT_SELECTOR');
    expect(runner).toContain('data-axhub-ai-generation-composer');
    expect(runner).toContain('data-axhub-prototype-composer');
    expect(runner).toContain('textarea[aria-label="AI 原型生成提示词"]');
    ...
    expect(runner).toContain("document.dispatchEvent(new CustomEvent('axhub:insertAiGeneration'");
