...
export interface RuntimeDevModuleRequestOptions {
  ...
  adminViteClientEnvPath?: string | null;
}
...
function getRequestReferer(headers: IncomingHttpHeaders | Record<string, string | string[] | undefined>): string {
  return getHeaderValue(headers.referer || headers.referrer).trim();
}

function getSearchParamFromUrl(value: string, key: string): string {
  try {
...
function getSearchParamFromRequestOrReferer(req: IncomingMessage, key: string): string {
...
function getFsPathFromRuntimePathname(pathname: string): string {
...
function isRuntimeGraphReferer(referer: string): boolean {
  if (!referer) {
    return false;
  }
  try {
...
function isRuntimeSourcePathname(pathname: string): boolean {
  const decodedPathname = decodePathname(pathname);
...
function isProjectScopedRuntimeSourceReferer(referer: string): boolean {
  if (!referer) {
    return false;
  }
  try {
...
function isActiveProjectFsRequest(requestUrl: string, runtimeProjectRoot?: string | null): boolean {
...
function isViteClientEnvRequest(requestUrl: string, adminViteClientEnvPath?: string | null): boolean {
...
function isRuntimePreviewPathname(pathname: string): boolean {
  const decodedPathname = decodePathname(pathname);
  return /(?:^|\/)(?:prototypes|themes)\//u.test(decodedPathname);
}

function isRuntimePreviewReferer(referer: string): boolean {
  if (!referer) {
    return false;
  }
  try {
...
export function isRuntimeDevModuleRequest(
  requestUrl: string,
  headers: IncomingHttpHeaders | Record<string, string | string[] | undefined>,
  options: RuntimeDevModuleRequestOptions = {},
): boolean {
...
export function isRuntimeDocumentRequest(requestUrl: string): boolean {
  const rawUrl = requestUrl || '/';
  if (/[?&]html-proxy\b/u.test(rawUrl)) {
    return false;
  }
...
export function createRuntimeUnavailableHtml(requestUrl: string, _detail = ''): string {
...
function sendRuntimeUnavailableHtml(
  res: ServerResponse,
  requestUrl: string,
  status: number,
  detail = '',
...
export function sendRuntimeUnavailableResponse(
  res: ServerResponse,
  requestUrl: string,
  status: number,
  detail = '',
...
function getForwardedHostForRuntime(req: IncomingMessage): string {
  return getFirstHeaderValue(req.headers['x-forwarded-host'])
    || getFirstHeaderValue(req.headers.host);
}

function getForwardedProtoForRuntime(req: IncomingMessage): string {
...
export function proxyRuntimeWebSocketUpgrade(
  req: IncomingMessage,
  socket: Duplex,
  head: Buffer,
