...
function createMinimalAxureExport(root: string) {
...
function runConverter(sourceRoot: string, outputName: string) {
...
function readLegacyPages(outputDir: string) {
...
describe('axure-html-converter', () => {
  ...
  it('preserves Axure body markup while executing only executable scripts through the controlled loader', () => {
    ...
    const homePage = pages.find((page: { id: string }) => page.id === 'page-001');
