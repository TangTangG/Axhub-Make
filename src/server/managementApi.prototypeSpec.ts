...
interface ResolvedPrototypeSpec {
  context: PrototypeSpecProjectContext;
  prototypeId: string;
  prototypeDir: string;
  specDir: string;
  apiBasePath: string;
...
function resolvePrototypeSpec(
  ...
  try {
    ...
    if (
      !isPathInside(realProjectRoot, realPrototypeDir)
  ...
  if (fs.existsSync(specDir)) {
    try {
      ...
      if (!isPathInside(realProjectRoot, realSpecDir) || !isPathInside(realPrototypeDir, realSpecDir)) return null;
      for (const mainSpecName of ['spec.html', 'spec.md']) {
        ...
        if (!isPathInside(realSpecDir, realMainSpecPath)) return null;
        const canonicalMainSpecPath = path.relative(realSpecDir, realMainSpecPath).split(path.sep).join('/');
        if (!normalizeSpecRelativePath(canonicalMainSpecPath)) return null;
      }
    } catch {
      return null;
    }
...
function descriptor(resolved: ResolvedPrototypeSpec): PrototypeSpecDescriptor {
  const isMainSpecFile = (name: string) => {
    try {
...
export function resolvePrototypeMainSpecStatus(
  context: PrototypeSpecProjectContext,
  prototypeId: string,
): PrototypeMainSpecStatus {
  const resolved = resolvePrototypeSpec(context, prototypeId, '');
  if (!resolved) return { available: false, activePath: null, projectPath: null };
...
function sendSpecError(res: ServerResponse, status: number, code: string, error: string): void {
  sendJson(res, { ok: false, code, error }, { status });
}

function normalizeSpecRelativePath(value: string): string | null {
  const raw = String(value || '').trim().replace(/\\/gu, '/');
  if (!raw || raw.includes('\0') || path.posix.isAbsolute(raw) || path.win32.isAbsolute(raw)) return null;
  const segments = raw.split('/').filter(Boolean);
  if (segments.length === 0 || segments.some((segment) => segment === '..' || segment === '.')) return null;
  if (PROTECTED_SPEC_DIRECTORIES.has(segments[0].toLowerCase())) return null;
  if (PROTECTED_SPEC_FILES.has(segments.at(-1)?.toLowerCase() || '')) return null;
  return segments.join('/');
}

function resolveSpecFile(resolved: ResolvedPrototypeSpec, value: string): string | null {
  const relativePath = normalizeSpecRelativePath(value);
  if (!relativePath) return null;
  const targetPath = path.resolve(resolved.specDir, relativePath);
  if (!isPathInside(resolved.specDir, targetPath)) return null;
  if (fs.existsSync(targetPath)) {
    try {
      const realSpecDir = fs.realpathSync(resolved.specDir);
      const realTargetPath = fs.realpathSync(targetPath);
      if (!isPathInside(realSpecDir, realTargetPath)) return null;
      const canonicalRelativePath = path.relative(realSpecDir, realTargetPath).split(path.sep).join('/');
      if (!normalizeSpecRelativePath(canonicalRelativePath)) return null;
    } catch {
      return null;
    }
  }
...
function resolveRequestedPath(resolved: ResolvedPrototypeSpec, requestedPath: string): string | null {
  if (requestedPath) return normalizeSpecRelativePath(requestedPath);
  return descriptor(resolved).activePath;
}

function buildRelativeSpecUrl(
  resolved: ResolvedPrototypeSpec,
  currentPath: string,
  rawValue: string,
  attributeName: string,
): { value: string; documentPath?: string } {
  ...
  if (!relativePath) return { value: rawValue };
...
export function handlePrototypeSpecApi(
  req: IncomingMessage,
  res: ServerResponse,
  options: ManagementApiOptions,
  pathname: string,
  handlers: PrototypeSpecApiHandlers,
): boolean {
