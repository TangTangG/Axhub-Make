...
function writeEnterpriseAuth(registryHome: string): void {
...
function writeAxhubPublishBinding(prototypeDir: string, binding: {
  pid: number;
  path: string;
  url: string;
  projectId: string;
...
describe('Axhub hosted review report APIs', () => {
  it('returns an explicit unbound state until the prototype is republished', async () => {
    ...
    try {
      const result = await fetch(
        `${server.origin}/api/review-reports/axhub-config?projectId=axhub-review-client&prototypeId=home`,
  ...
  it('reads and updates the bound hosted submission config through the Axhub account', async () => {
    ...
    const originalFetch = globalThis.fetch;
    const upstreamRequests: Array<{ url: string; method: string; body: unknown }> = [];
    vi.stubGlobal('fetch', vi.fn(async (input: string | URL | Request, init?: RequestInit) => {
      const requestUrl = String(input);
      if (!requestUrl.startsWith('https://enterprise.test/')) {
        return originalFetch(input, init);
  ...
  it('downloads hosted reports to stable local files with Axhub identity metadata', async () => {
    ...
    try {
      ...
      const listed = await fetch(
        `${server.origin}/api/review-reports?projectId=axhub-review-client&prototypeId=home`,
      ).then(readJsonResponse);
