...
import {
  PREVIEW_BRIDGE_WS_PATH,
  PreviewBridgeHub,
  isPreviewBridgeUpgrade,
...
function findServerTextFrames(socket: FakeSocket): Buffer[] {
...
function connectPreviewClient(hub: PreviewBridgeHub): FakeSocket {
...
describe('preview bridge websocket hub', () => {
  ...
  it('matches only the preview bridge websocket path', () => {
    ...
    expect(isPreviewBridgeUpgrade({ url: '/ws/preview-bridge?client=1' } as any)).toBe(true);
    expect(isPreviewBridgeUpgrade({ url: '/ws/canvas-bridge' } as any)).toBe(false);
    expect(isPreviewBridgeUpgrade({ url: '' } as any)).toBe(false);
    hub.destroy();
  });
});
