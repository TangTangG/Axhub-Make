...
function writePrototype(projectRoot: string, prototypeId = 'home'): string {
...
function getReviewConfigPath(prototypeDir: string): string {
...
function writeMultipartBody(params: {
  ...
  files: Array<{ fieldName: string; fileName: string; content: string; contentType?: string }>;
}): string {
  return [
...
describe('review report APIs', () => {
  ...
  it('lists current-prototype review reports by newest time and reads markdown details', async () => {
    ...
    try {
      ...
      expect(list.body.reports.map((report: any) => report.id)).not.toContain('ui-review');
      const invalidScoreReport = list.body.reports.find((report: any) => report.id === 'invalid-score-review');
  ...
  it('lets enabled LAN report submission bypass the global LAN password without opening other review APIs', async () => {
    ...
    try {
      ...
      const protectedList = await fetch(
        `${server.origin}/api/review-reports?projectId=review-lan-bypass-client&prototypeId=home`,
        { headers: nonLocalHeaders },
      ).then(readJsonResponse);
      expect(protectedList).toMatchObject({ status: 401, body: { code: 'LAN_AUTH_REQUIRED' } });

      const protectedDetail = await fetch(
        `${server.origin}/api/review-reports/existing-report?projectId=review-lan-bypass-client&prototypeId=home`,
        { headers: nonLocalHeaders },
      ).then(readJsonResponse);
      ...
      const exists = await fetch(
        `${server.origin}/api/review-reports/exists?projectId=review-lan-bypass-client&prototypeId=home&reportId=bypassed-lan-submit`,
        { headers: nonLocalHeaders },
      ).then(readJsonResponse);
      ...
      const missing = await fetch(
        `${server.origin}/api/review-reports/exists?projectId=review-lan-bypass-client&prototypeId=home&reportId=missing-report`,
        { headers: nonLocalHeaders },
      ).then(readJsonResponse);
