...
import {
  ...
  type AxhubHostedReviewConfig,
  type AxhubHostedReviewReport,
...
function sendError(res: ServerResponse, error: any, fallback: string): void {
...
function getPrototypeContext(
  res: ServerResponse,
  context: AxhubReviewProjectContext,
  prototypeId: string,
): string | null {
...
function createConfigResponse(
  context: AxhubReviewProjectContext,
  prototypeId: string,
  remote?: AxhubHostedReviewConfig,
) {
...
function createSyncedReportMarkdown(report: AxhubHostedReviewReport): string {
...
function readSyncedPayloadHash(filePath: string): string {
...
function syncReportsToPrototype(prototypeDir: string, reports: AxhubHostedReviewReport[]) {
...
export function handleAxhubReviewReportsApi(
  req: IncomingMessage,
  res: ServerResponse,
  options: ManagementApiOptions,
  pathname: string,
  url: URL,
  handlers: AxhubReviewApiHandlers,
): boolean {
  if (pathname === '/api/review-reports/axhub-sync' && req.method === 'POST') {
    readJsonBody<Record<string, unknown>>(req).then(async (body) => {
      ...
      const reports = remote.reports.filter((report) => (
        report.projectId === local.binding.projectId
  ...
  if (req.method === 'PUT') {
    readJsonBody<Record<string, unknown>>(req).then(async (body) => {
      ...
      const remote = await createClient(options).updateHtmlProjectReviewConfig(
        local.binding.pid,
        body.submitEnabled === true,
      );
