...
export interface AcpChatCommandRequest {
  acpApiBaseUrl: string;
  id?: string;
  threadId?: string;
  provider?: unknown;
...
export interface AcpRuntimeHeaders {
  provider?: string;
  threadId?: string;
  sessionKey?: string;
  sessionId?: string;
...
export interface AcpToolOutputChunk {
...
export interface AcpChatRunErrorEntry {
  type: 'error' | 'tool-output-error';
  message: string;
  toolCallId?: string;
  toolName?: string;
  chunk?: Record<string, unknown>;
...
export interface AcpChatRunResult {
  success: true;
  id: string;
  threadId: string;
  provider: string;
...
export type AcpChatStreamEvent =
  | {
      type: 'start';
      result: AcpChatRunResult;
...
function buildAcpChatUrl(acpApiBaseUrl: string): string {
...
function captureRuntimeHeaders(headers: Headers): AcpRuntimeHeaders {
  const raw = Object.fromEntries(
    RUNTIME_HEADER_KEYS
      .map((key) => [key, readHeader(headers, key)] as const)
      .filter(([, value]) => value),
  );
  const runtimeHeaders: AcpRuntimeHeaders = { raw };
  ...
  if (provider) runtimeHeaders.provider = provider;
  if (threadId) runtimeHeaders.threadId = safeDecodeURIComponent(threadId);
  ...
  if (modeId) runtimeHeaders.modeId = modeId;
  if (thoughtLevel) runtimeHeaders.thoughtLevel = thoughtLevel;
  if (warningCount !== undefined) runtimeHeaders.warningCount = warningCount;
...
export function createAcpOneShotThreadId(prefix: unknown = 'exec'): string {
  const normalizedPrefix = normalizeOptionalId(prefix) || 'exec';
  const suffix = crypto.randomUUID().replace(/[^A-Za-z0-9_-]/gu, '');
  return `${normalizedPrefix}-${suffix}`;
}

function normalizeMessages(value: unknown): AcpChatMessage[] {
  return Array.isArray(value) ? value as AcpChatMessage[] : [];
}
...
function recordChunk(result: AcpChatRunResult, chunk: Record<string, unknown>): void {
...
async function* readSseJson(response: Response): AsyncGenerator<Record<string, unknown>> {
  if (!response.body) {
...
function buildAbortSignal(options: AcpChatRunnerOptions): AbortSignal | undefined {
...
async function startAcpChatRequest(
  ...
  if (request.permissionMode !== undefined) body.permissionMode = request.permissionMode;
  if (request.mcpServers !== undefined) body.mcpServers = request.mcpServers;
  if (request.builtinTools !== undefined) body.builtinTools = request.builtinTools;
  if (request.builtinToolSettings !== undefined) body.builtinToolSettings = request.builtinToolSettings;
...
export async function* streamAcpChat(
  request: AcpChatRequest,
  options: AcpChatRunnerOptions = {},
): AsyncGenerator<AcpChatStreamEvent> {
...
export async function runAcpChatCommand(
  request: AcpChatCommandRequest,
  options: AcpChatRunnerOptions = {},
): Promise<AcpChatRunResult> {
